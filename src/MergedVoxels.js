...
(function () {
    ...
    MergedVoxels.prototype.FindAndSetPointBottom = function (bound, map, x, y, z, to) {// 查找同一面的右下角定点和右上角定点
        ...
        for (zz = z + 1; zz <= bound.maxZ; zz++) {
            ...
            for (xx = x; xx <= to.x; xx++) { // bottom to up
                if (!this.isSameMaterial(map, xx, y, zz, material) || map[y][zz][xx][face]) {
                    noSame = true;
                    break;
                }
            }
    ...
    MergedVoxels.prototype.FindAndSetPointRight = function (bound, map, x, y, z, to) {// 查找同一面的右下角定点和右上角定点
        ...
        for (yy = y + 1; yy <= bound.maxY; yy++) {
            ...
            for (zz = z; zz >= to.z; zz--) { // bottom to up
                if (!this.isSameMaterial(map, x, yy, zz, material) || map[yy][zz][x][face]) {
                    noSame = true;
                    break;
                }
            }
    ...
    MergedVoxels.prototype.FindAndSetPointBack = function (bound, map, x, y, z, to) { // find from left-bottom to right-top in back face
        ...
        for (yy = y + 1; yy <= bound.maxY; yy++) {
            noSame = false;
            for (xx = x; xx >= to.x; xx--) { // bottom to up
                if (!this.isSameMaterial(map, xx, yy, z, material) || map[yy][z][xx][face]) {
                    noSame = true;
                    break;
                }
            }
